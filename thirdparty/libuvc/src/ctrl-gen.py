...
def gen_decl(unit_name, unit, control_name, control):
    fields = [(load_field(field_name, field_details), field_details['doc']) for field_name, \
              field_details in control['fields'].items()] if 'fields' in control else []
...
def gen_tool_help(unit_name, unit, control_name, control):
        fields = [(load_field(field_name, field_details), field_details['doc']) for field_name, \
                field_details in control['fields'].items()] if 'fields' in control else []

        args_doc = "\n".join(["    printf(\"    {0}: [{1}]\\n\");".format(field.name, desc) for (field, desc) in fields])
...
def usage():
    print("Usage: python ctrl-gen.py [options] [mode]")
    print("Options:")
    print("  -h, --help         Show this help message and exit")
    print("  -i, --input FILE   Input YAML file(s) containing control definitions")
    print("Modes:")
    print("  def                Generate C function definitions and output to a .c file")
    print("  decl               Generate C function declarations and output to a .h file")
    print("  yaml               Generate YAML output from the input YAML files")
    print("Examples:")
    print("  Generate C function definitions and output to ctrl-gen.c:")
    print("    python ctrl-gen.py -i ../standard-units.yaml def > ctrl-gen.c")
    print("  Generate C function declarations and output to ctrl-gen.h:")
    print("    python ctrl-gen.py -i ../standard-units.yaml decl > ctrl-gen.h(Need to manually paste to libuvc.h)")
    print("  Generate YAML output:")
    print("    python ctrl-gen.py -i ../standard-units.yaml yaml > output.yaml")
    print("  Process multiple YAML files:")
    print("    python ctrl-gen.py -i ../standard-units.yaml -i ../custom-units.yaml def > ctrl-gen.c")
if __name__ == '__main__':
    ...
    mode = None
    for arg in args:
        if arg in ('def', 'decl', 'yaml', 'tool_help'):
            if mode is None:
                mode = arg
            else:
...
""")
        ...
        print("""/* This is an AUTO-GENERATED file! Update it with the output of `ctrl-gen.py tool_help`. */

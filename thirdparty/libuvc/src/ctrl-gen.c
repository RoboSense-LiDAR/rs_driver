...
/** @ingroup ctrl
 * @brief Reads the TRANSFER_MODE control.
 * @param devh UVC device handle
 * @param[out] transfer_mode 0: usb, 1: ethernet
 * @param req_code UVC_GET_* request to execute
 */
uvc_error_t uvc_get_transfer_mode(uvc_device_handle_t *devh, uint8_t* transfer_mode, enum uvc_req_code req_code) {
  uint8_t data[1];
  uvc_error_t ret;
  ...
  ret = libusb_control_transfer(
    devh->usb_devh,
    REQ_TYPE_GET, req_code,
    UVC_XU_TRANSFER_MODE_CONTROL << 8,
...
/** @ingroup ctrl
 * @brief Sets the TRANSFER_MODE control.
 * @param devh UVC device handle
 * @param transfer_mode 0: usb, 1: ethernet
 */
uvc_error_t uvc_set_transfer_mode(uvc_device_handle_t *devh, uint8_t transfer_mode) {
  uint8_t data[1];
  uvc_error_t ret;

  data[0] = transfer_mode;
...
/** @ingroup ctrl
 * @brief Reads the PC_FPS control.
 * @param devh UVC device handle
 * @param[out] pc_fps 10: 10Hz, 20: 20Hz, 30: 30Hz
 * @param req_code UVC_GET_* request to execute
 */
uvc_error_t uvc_get_point_cloud_fps(uvc_device_handle_t *devh, uint8_t* pc_fps, enum uvc_req_code req_code) {
  uint8_t data[1];
  uvc_error_t ret;
...
/** @ingroup ctrl
 * @brief Sets the PC_FPS control.
 * @param devh UVC device handle
 * @param pc_fps 10: 10Hz, 20: 20Hz, 30: 30Hz
 */
uvc_error_t uvc_set_point_cloud_fps(uvc_device_handle_t *devh, uint8_t pc_fps) {
  uint8_t data[1];
  uvc_error_t ret;
...
/** @ingroup ctrl
 * @brief Reads the USB_MODE_CFG control.
 * @param devh UVC device handle
 * @param[out] usb_mode_cfg 0: uvc, 1: adb, 2: eth_bridge
 * @param req_code UVC_GET_* request to execute
 */
uvc_error_t uvc_get_usb_mode_cfg(uvc_device_handle_t *devh, uint8_t* usb_mode_cfg, enum uvc_req_code req_code) {
  uint8_t data[1];
  uvc_error_t ret;
...
/** @ingroup ctrl
 * @brief Sets the USB_MODE_CFG control.
 * @param devh UVC device handle
 * @param usb_mode_cfg 0: uvc, 1: adb, 2: eth_bridge
 */
uvc_error_t uvc_set_usb_mode_cfg(uvc_device_handle_t *devh, uint8_t usb_mode_cfg) {
  uint8_t data[1];
  uvc_error_t ret;
...
/** @ingroup ctrl
 * @brief Reads the SYS_TIME control.
 * @param devh UVC device handle
 * @param[out] sys_time 1: set x3 system time
 * @param req_code UVC_GET_* request to execute
 */
uvc_error_t uvc_get_sys_time(uvc_device_handle_t *devh, uint8_t* sys_time, enum uvc_req_code req_code) {
  uint8_t data[16];
  uvc_error_t ret;
  struct timespec ts;
  ...
  if (ret == sizeof(data)) {
    *sys_time = data[0];
    ts.tv_sec = (uint64_t)((uint64_t)data[7] << 56 | (uint64_t)data[6] << 48 | (uint64_t)data[5] << 40 | (uint64_t)data[4] << 32 
                            | (uint64_t)data[3] << 24 | (uint64_t)data[2] << 16 | (uint64_t)data[1] << 8 | (uint64_t)data[0]);
    ts.tv_nsec = (uint64_t)((uint64_t)data[15] << 56 | (uint64_t)data[14] << 48 | (uint64_t)data[13] << 40 | (uint64_t)data[12] << 32 
                            | (uint64_t)data[11] << 24 | (uint64_t)data[10] << 16 | (uint64_t)data[9] << 8 | (uint64_t)data[8]);
    printf("sys_time :%ld.%ld\r\n",ts.tv_sec,ts.tv_nsec);
    return UVC_SUCCESS;
  } else {
    return ret;
  }
